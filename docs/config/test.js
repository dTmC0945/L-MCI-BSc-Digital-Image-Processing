...
function OrgNode(
  ...
  if (null != folder) t.FOLDER = folder;
...
OrgNode.hideElement = function (e) {
  if (e && e["style"]) {
    // test for e['style'] is just quick fix for error elsewhere (fixed toc and title)
...
OrgNode.textNodeToIdx = function (dom, org) {
  while (
    dom.nodeType != 1 /* IE has no Node.ELEMENT_NODE... */ ||
...
OrgNode.prototype.show = function () {
  ...
  if (this.DEPTH > 2) this.PARENT.show();
};
...
var org_html_manager = {
  ...
  ORGTAG_REGEX:
    /^(.*)<span\s+class=[\'\"]tag[\'\"]>(<span[^>]+>[^<]+<\/span>)+<\/span>/i, // Remove Org tags
  TRIMMER: /^(\s*)([^\s].*)(\s*)$/, // Trim
  ...
  SEARCH_REGEX: "",
  SEARCH_HL_REGEX: new RegExp(
    '(<span class="org-info-js_search-highlight">)([^<]*?)(</span>)',
    "gi",
  ),
  ...
  setup: function () {
    ...
    if (location.search) {
      // enable overwriting of settings
    ...
    t.STARTUP_MESSAGE =
      t.STARTUP_MESSAGE && t.STARTUP_MESSAGE != "0" ? true : false;
    t.LOCAL_TOC = t.LOCAL_TOC && t.LOCAL_TOC != "0" ? t.LOCAL_TOC : false;
    t.HIDE_TOC = t.TOC && t.TOC != "0" ? false : true;
    t.INNER_TITLE =
      t.INNER_TITLE && t.INNER_TITLE != "title_above" ? false : true;
    ...
    t.LINKS +=
      (t.LINK_UP && t.LINK_UP != document.URL
        ? '<a href="' + t.LINK_UP + '">Up</a> / '
        : "") +
      (t.LINK_HOME && t.LINK_HOME != document.URL
  ...
  init: function () {
    ...
    if (null == t.BODY) {
      if (5 > t.RUNS) {
        t.LOAD_CHECK = window.setTimeout(
    ...
    t.CONSOLE.innerHTML =
      '<form action="" style="margin:0px;padding:0px;" onsubmit="org_html_manager.evalReadCommand(); return false;">' +
    ...
    if (t.VIEW == t.INFO_VIEW) {
      ...
      t.toggleGlobaly();
      if (v > t.PLAIN_VIEW) t.toggleGlobaly();
      if (v == t.ALL_VIEW) t.toggleGlobaly();
    ...
    if (t.STARTUP_MESSAGE) {
      t.warn(
        "This page uses org-info.js. Press '?' for more information.",
        true,
      );
    }
  ...
  initFromTOC: function () {
    ...
    // Return false if not.
    if (!t.ulToOutlines(theIndex)) return false;
    ...
    if (ul.hasChildNodes()) {
      for (var i = 0; i < ul.childNodes.length; ++i) {
        ...
        for (var j = 0; j < li.childNodes.length; ++j) {
          ...
          if (c.nodeName == "UL") {
            if (cur_depth > this.TOC_DEPTH) li.removeChild(c);
            else this.cutToc(c, cur_depth);
          }
        }
      }
    }
  ...
  build: function () {
    ...
    for (var i = 0; i < this.SECS.length; ++i) {
      this.SECS[i].IDX = i;
      var html =
        '<table class="org-info-js_info-navigation" width="100%" border="0" style="border-bottom:0px solid black;">' +
        '<tr><td colspan="3" style="text-align:left;border-style:none;vertical-align:bottom;padding-bottom:10px;">' +
        '<span style="float:left;display:inline;text-align:left;">' +
        'Top: <a accesskey="t" href="javascript:org_html_manager.navigateTo(0);">' +
        index_name +
        "</a></span>" +
        '<span style="float:right;display:inline;text-align:right;font-size:70%;">' +
        this.LINKS +
        '<a accesskey="m" href="javascript:org_html_manager.toggleView(' +
        i +
        ');">toggle view</a></span>' +
        '</td></tr><tr><td style="text-align:left;border-style:none;vertical-align:bottom;width:22%;padding-top:10px;padding-bottom:10px;padding-left:10px;">';
      ...
      if (i > 0)
        html +=
          ...
          this.SECS[i - 1].L +
          '" title="Go to: ' +
          this.removeTags(this.SECS[i - 1].HEADING.innerHTML) +
          '">Previous</a> | ';
      else html += "Previous | ";
      ...
      html +=
        '</td><td style="text-align:center;vertical-align:bottom;border-style:none;width:56%;padding-bottom:10px;">';

      if (i > 0 && this.SECS[i].PARENT.PARENT)
        // != this.ROOT)
        html +=
          '<a href="' +
          this.SECS[i].PARENT.L +
          '" title="Go to: ' +
          this.removeTags(this.SECS[i].PARENT.HEADING.innerHTML) +
          '">' +
        ...
        html +=
          '<span style="font-style:bold;">' +
          this.SECS[i].HEADING.innerHTML +
          "</span>";

      // Right:
      html +=
        '</td><td style="text-align:right;vertical-align:bottom;border-style:none;width:22%;padding-bottom:10px;padding-right:10px;">';
      html += i + 1 + "</td></tr></table>";

      // buttons:
      this.SECS[i].BUTTONS = document.createElement("div");
      this.SECS[i].BUTTONS.innerHTML =
        '<div class="org-info-js_header-navigation" style="display:inline;float:right;text-align:right;font-size:70%;font-weight:bold;padding-top:10px;">' +
        this.LINKS +
        '<a accesskey="m" href="javascript:org_html_manager.toggleView(' +
        i +
        ');">toggle view</a></div>';
      ...
      if (0 < this.SECS[i].CHILDREN.length && this.LOCAL_TOC) {
        ...
        for (var k = 0; k < this.SECS[i].CHILDREN.length; ++k) {
          html +=
            ...
            '">' +
            this.removeTags(
              this.removeOrgTags(this.SECS[i].CHILDREN[k].HEADING.innerHTML),
            ) +
            "</a></li>";
        }
        html += "</ul>";
        ...
        if ("above" == this.LOCAL_TOC) {
          if (this.SECS[i].FOLDER)
            this.SECS[i].FOLDER.insertBefore(
              navi2,
              this.SECS[i].FOLDER.firstChild,
            );
          else
            this.SECS[i].DIV.insertBefore(
              navi2,
              this.SECS[i].DIV.getElementsByTagName("h" + this.SECS[i].DEPTH)[0]
                .nextSibling,
            );
        } else {
          if (this.SECS[i].FOLDER) this.SECS[i].FOLDER.appendChild(navi2);
          else this.SECS[i].DIV.appendChild(navi2);
        }
      }
    }
  ...
  set: function (eval_key, eval_val) {
    if ("VIEW" == eval_key) {
      ...
      if (eval_val) eval("this." + eval_key + "='" + eval_val + "';");
      else eval("this." + eval_key + "=0;");
    }
  },
  ...
  showSection: function (sec) {
    ...
    if ("?/toc/?" == sec || (!isNaN(section) && t.SECS[section])) {
      if ("?/toc/?" == sec && t.HIDE_TOC) {
        ...
        if (t.INFO_VIEW == t.VIEW) t.WINDOW.innerHTML = t.NODE.DIV.innerHTML;
        else t.NODE.setState(OrgNode.STATE_UNFOLDED);
        window.scrollTo(0, 0);
      } else {
          ...
          if (last_node.IDX != t.NODE.IDX)
            if ("?/toc/?" != sec)
              window.location.replace(
                t.BASE_URL + t.Q_PARAM + t.getDefaultTarget(),
              );
  ...
  plainView: function (sec, skip_show_section) {
    ...
    if (t.WINDOW.firstChild)
      // might not be set after init
  ...
  adjustSlide: function (sec, show) {
    ...
    if (next) {
      for (
        var n = this.SECS[sec].FOLDER.firstChild;
        null != n;
        n = n.nextSibling
      ) {
        if ("UL" == n.nodeName) {
    ...
    if (nextForward)
      document.onclick = function () {
        org_html_manager.scheduleClick(
          "org_html_manager.nextSection(org_html_manager.NODE.IDX + 1)",
        );
      };
    else
      document.onclick = function () {
        org_html_manager.scheduleClick(
          "org_html_manager.adjustSlide(org_html_manager.NODE.IDX, +1)",
        );
      };
    if (nextBack)
      ...
      document.ondblclick = function () {
        org_html_manager.scheduleClick(
          "org_html_manager.adjustSlide(" + this.NODE.IDX + ", -1)",
        );
      };
  },
  ...
  toggleView: function (sec) {
    ...
    t.Q_PARAM = "";
    t.removeWarning();
    if (t.VIEW == t.INFO_VIEW) t.plainView(sec);
    else t.infoView(sec);
  },
  ...
  scheduleClick: function (func, when) {
    ...
    if (null == this.CLICK_TIMEOUT) {
      this.CLICK_TIMEOUT = window.setTimeout(
        "org_html_manager.executeClick(" + func + ")",
        when,
      );
    } else {
  ...
  popHistory: function (foreward) {
    ...
    if (foreward) {
      if (t.HISTORY[t.HIST_INDEX]) {
        ...
        t.HFO = 1;
        t.warn(
          "History: No where to foreward go from here. Any key and `B' to move to next file in history.",
        );
      }
    } else {
      if (t.HISTORY[t.HIST_INDEX - 1]) {
        ...
        t.HBO = 1;
        t.warn(
          "History: No where to back go from here. Any key and `b' to move to previous file in history.",
        );
      }
    }
  },
  ...
  getKey: function () {
    ...
    if (1 == s.length) {
      ...
      if ("b" == s) {
        ...
        if (window.innerHeight) window.scrollBy(0, window.innerHeight - 30);
        else if (document.documentElement.clientHeight)
          window.scrollBy(0, document.documentElement.clientHeight - 30);
        else window.scrollBy(0, document.body.clientHeight - 30);
      } else if ("V" == s) {
        if (window.innerHeight) window.scrollBy(0, -(window.innerHeight - 30));
        else if (document.documentElement.clientHeight)
          window.scrollBy(0, -(document.documentElement.clientHeight - 30));
      ...
      /* === READ COMMANDS === */
        if ("" != t.OCCUR) {
          t.startRead(
            t.READ_COMMAND_HTML_LINK,
            "Choose HTML-link type: 's' = section, 'o' = occur",
          );
        } else {
          t.startRead(
            s,
            ...
            '<a href="' +
              ...
              document.title +
              ", Sec. '" +
              t.removeTags(t.NODE.HEADING.innerHTML) +
              "'</a>",
            "C-c to copy, ",
          );
          window.setTimeout(function () {
        ...
        if ("" != t.OCCUR) {
          t.startRead(
            t.READ_COMMAND_ORG_LINK,
            "Choose Org-link type: 's' = section, 'o' = occur",
          );
        } else {
          t.startRead(
            s,
        ...
        if ("" != t.OCCUR)
          t.startRead(s, "Occur:", t.OCCUR, "RET to use previous, DEL ");
  ...
  evalReadCommand: function () {
    ...
    if (command == "g") {
      ...
      if ("s" == c) {
        ...
        t.startRead(
          t.READ_COMMAND_NULL,
          "Org-link, occurences of <i>&quot;" + t.OCCUR + "&quot;</i>:",
          "[[" +
            t.BASE_URL +
            "?OCCUR=" +
            t.OCCUR +
      ...
      if ("s" == c) {
        ...
        t.startRead(
          t.READ_COMMAND_NULL,
          "HTML-link, occurences of <i>&quot;" + t.OCCUR + "&quot;</i>:",
          '<a href="' +
            t.BASE_URL +
            "?OCCUR=" +
            t.OCCUR +
      ...
      if ("s" == c) {
        ...
        t.startRead(
          t.READ_COMMAND_NULL,
          "Plain-link, occurences of <i>&quot;" + t.OCCUR + "&quot;</i>:",
  ...
  highlightHeadline: function (h) {
    ...
    if (this.PLAIN_VIEW == this.VIEW && this.MOUSE_HINT) {
      ...
      else this.SECS[i].HEADING.style.backgroundColor = this.MOUSE_HINT;
    }
  },
  ...
  unhighlightHeadline: function (h) {
    ...
    } else this.SECS[i].HEADING.style.backgroundColor = "";
  },
    ...
    if (t.HELPING) {
      ...
      if (null == t.TAGS_INDEX) {
        ...
        for (var i = 0; i < t.SORTED_TAGS.length; ++i) {
          ...
          t.TAGS_INDEX +=
            "<a href=\"javascript:OrgNode.toggleElement(document.getElementById('" +
            fid +
            "'));\"><h3>" +
            tag +
            "</h3></a>" +
          ...
          for (var j = 0; j < t.TAGS[tag].length; ++j) {
            ...
            t.TAGS_INDEX +=
              '<li><a href="javascript:org_html_manager.showSection(' +
              idx +
              ');">' +
              t.SECS[idx].HEADING.innerHTML +
              "</a></li>";
          }
          t.TAGS_INDEX += "</ul></div>";
        }
...
function OrgHtmlManagerKeyEvent(e) {
  ...
  if (e.shiftKey)
    org_html_manager.CONSOLE_INPUT.value =
      org_html_manager.CONSOLE_INPUT.value + s;
  else
    org_html_manager.CONSOLE_INPUT.value =
      org_html_manager.CONSOLE_INPUT.value + s.toLowerCase();
...
function toggle_visibility() {
  var elements = document.getElementsByClassName("pre"),
    n = elements.length;
  for (var i = 0; i < n; i++) {
    var e = elements[i];
...
orgInfoHooks = {
  onShowSection: [toggle_visibility],
};
/* ]]> */
